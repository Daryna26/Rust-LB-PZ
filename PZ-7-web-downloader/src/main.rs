...
#[command(about = "Async CLI program for downloading web pages")]
struct Cli {
    #[arg(long)]
...
async fn download_page(client: Client, url: String, index: usize) -> Result<()> {
    ...
    println!("Збережено: {file_name}");

    Ok(())
}

async fn run_downloader(links: Vec<String>) -> Result<()> {
    if links.is_empty() {
    ...
    let tasks = links
        .into_iter()
        .enumerate()
        .map(|(index, url)| download_page(client.clone(), url, index + 1));
...
fn main() -> Result<()> {
    ...
    runtime.block_on(run_downloader(links))?;
